...
ggplot(data = Scenario1.table, aes(x = as.factor(Trials), y = MSE, color = Method)) +
  geom_point() +
...
ggplot(data = Scenario1.Error, aes(x = as.factor(Trials), y = Error, fill = Partial)) +
...
ggplot(data = Scenario1.Error2, aes(x = as.factor(Noise), y = Error, fill = Partial)) +
  geom_bar(stat = "identity", position = "dodge") +
  theme_minimal() +
...
ggplot(data = Scenario1.SDI, aes(x = as.factor(Trials), y = SDI, color = as.factor(Noise))) +
  geom_point() +
  theme_minimal() +
