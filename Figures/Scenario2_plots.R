...
ggplot(data = Scenario2.NoiseBreakdown, aes(x = as.factor(Trials), y = Distortion)) +
  geom_point(aes(color = as.factor(Noise))) +
  theme_minimal() +
  labs(title = "Distortion across Trials and Noise Levels for Scenario 2", x = "Number of Trials",
       y = "Distortion", color = "Noise Level") +
  theme(plot.title = element_text(hjust = 0.5))

ggplot(data = Scenario2.NoiseBreakdown, aes(x = as.factor(Trials), y = ResidualNoise)) +
  geom_point(aes(color = as.factor(Noise))) +
  theme_minimal() +
  labs(title = "Residual Noise across Trials and Noise Levels for Scenario 2", x = "Number of Trials",
